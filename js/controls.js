// js/controls.js
// ---------------------------------------------------------------------
// Touch controls for the voxel engine (Bedrock style).
//
// Integration contract with the engine (classic script game.js):
//   - Keyboard: the engine reads canvas.onkeydown / canvas.onkeyup and
//     normalizes with e.key.toLowerCase(), storing state in a Key table.
//     We dispatch synthetic KeyboardEvents on window.canvas so every
//     virtual key goes through the exact same handlers as a real key.
//   - Mouse: the engine tick loop performs hold-to-break / hold-to-place
//     while Key.leftMouse / Key.rightMouse are true, gated internally by
//     screen === "play" and 250ms cooldowns (lastBreak / lastPlace).
//     The engine exposes win.Key and window.player, so the attack/place
//     buttons simply flip those states and let the engine do the rest.
//   - Look: a canvas drag rotates window.player with the same math as the
//     engine's pointer-lock mouse handler (mmoved).
//
// This module never modifies engine physics; it only feeds input.
// ---------------------------------------------------------------------

var HALF_PI = Math.PI / 2
var TWO_PI = Math.PI * 2

// Buttons mapped to synthetic keyboard events (key = e.key as the engine expects)
var KEY_BUTTONS = [
...
// Buttons mapped to engine mouse states (hold-to-break / hold-to-place)
var MOUSE_BUTTONS = [
...
export class TouchController {
	constructor(options) {
		var o = options || {}
		this.deadZone = o.deadZone !== undefined ? o.deadZone : 0.25
		this.engageAt = o.engageAt !== undefined ? o.engageAt : 0.40
		this.releaseAt = o.releaseAt !== undefined ? o.releaseAt : 0.25
		this.sprintEngageAt = o.sprintEngageAt !== undefined ? o.sprintEngageAt : -0.95
		this.sprintReleaseAt = o.sprintReleaseAt !== undefined ? o.sprintReleaseAt : -0.80
		this.lookSensitivity = o.lookSensitivity || 0.005
		this.joyRadius = 50
		this.attached = false
		this.els = {}
		this._listeners = []
	...
	get isTouchDevice() {
		// Manual override for DevTools testing: index.html?touch=1
	...
	addListener(el, type, fn) {
		...
		this._listeners.push({ el: el, type: type, fn: fn, opts: { passive: false } })
	}
	...
	moveThumb(x, y) {
		...
		var nx = ndx / r
		var ny = ndy / r
		// Independent axes -> 8 directions, hysteresis per key.
		this.applyAxis("w", "s", ny)
		this.applyAxis("a", "d", nx)
		// Bedrock "Sprint using the joystick": pushing to the top edge sprints.
	...
	setDirKey(key, on) {
		...
		if (on) {
			// Re-emit if the engine cleared the table (e.g. pointerlockchange)
	...
	bindLook() {
		...
		var move = function(e) {
			// A moving finger is a camera drag, never a click.
			e.preventDefault()
			if (self.lookTouchId === null) return
			...
			self.lookLast = { x: t.clientX, y: t.clientY }
			if (!p) return
			// Same math as the engine's pointer-lock handler (mmoved)
		...
		for (var i = 0; i < slots.length; i++) {
			(function(slot, n) {
				...
				self.addListener(slot, "touchstart", function(e) {
					...
					// The engine keydown handler updates inventory.hotbarSlot
					// and holding for number keys, then keyup clears the state.
					self.emitKey(key, true, "Digit" + key)
					self.emitKey(key, false, "Digit" + key)
					setTimeout(function() { slot.classList.remove("pressed") }, 150)
				})
			})(slots[i], i + 1)
		}
	}
